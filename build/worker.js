...
/**
 * TODO: This is how the worker takes the source and run it. Need to figure out
 * how to make it work with the new setup in nodejs.
 */
...
function makeDefaultedMaterial(doc, matIn = {}) {
  ...
  const material = doc.createMaterial(
    (_a = matIn.name) !== null && _a !== void 0 ? _a : ""
  );
  if (unlit) {
    const unlit = doc
      .createExtension(extensions_1.KHRMaterialsUnlit)
      .createUnlit();
...
function addMesh(doc, to3mf, node, manifold, backupMaterial = {}) {
  ...
  log(
    `Genus: ${manifold.genus().toLocaleString()}, Volume: ${(
      volume / 100
    ).toLocaleString()} cm^3`
  );
  ...
  for (const id of manifoldMesh.runOriginalID) {
    ...
    id2properties.set(id, {
      ...
      attributes: [
        "POSITION",
        ...((_a = material.attributes) !== null && _a !== void 0 ? _a : []),
      ],
    });
  }
  const morph = manifold2morph.get(manifold);
  ...
  for (const [run, id] of manifoldMesh.runOriginalID.entries()) {
    ...
    id2properties.get(id).material = getCachedMaterial(
      doc,
      show ? SHOW : id2material.get(id) || backupMaterial
    );
    const debugNode = doc
      .createNode("debug")
  ...
  return __awaiter(this, void 0, void 0, function* () {
    ...
    if (nodes.length > 0) {
      ...
      for (const nodeDef of nodes) {
        node2gltf.set(
          nodeDef,
          createNodeFromCache(doc, to3mf, nodeDef, manifold2node)
        );
        if (nodeDef.manifold) {
          ++leafNodes;
        }
