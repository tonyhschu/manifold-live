...
import { Mesh as ManifoldMesh } from "manifold-3d";
import { MeshOptions } from "manifold-3d/manifold-encapsulated-types";
...
export function readMesh(
  ...
  const mergeFromVertArray: number[] = [];
  const mergeToVertArray: number[] = [];
